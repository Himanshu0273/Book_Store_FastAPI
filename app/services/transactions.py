...
from app.utils.enums import TransactionStatusEnum, PaymentsEnum, CartActivityEnum
...
def add_transaction(payment: Payments, db: Session) -> Transactions | None:
    ...
    try:
        ...
        message = (
            "Transaction Successful" if status == TransactionStatusEnum.SUCCESS
        ...
        func_logger.info(f"Transaction {transaction.id} created for Payment ID: {payment.id} with status: {status}")
        return transaction

    except SQLAlchemyError as e:
        db.rollback()
        func_logger.error(f"DB error during transaction for Payment ID: {payment.id} | Error: {e}")
        return None

    except Exception as e:
        db.rollback()
        func_logger.exception(f"Unexpected error during transaction creation for Payment ID: {payment.id}")
        return None
...
def rollback_payment(ord_id: int, user_id: int, payment: Payments, db: Session):
    try:
        func_logger.info(f"Rolling back Payment ID: {payment.id} for Order ID: {ord_id} and User ID: {user_id}")
        if payment.attempts >= 3:
            func_logger.warning(f"Payment ID: {payment.id} has already reached max attempts.")
            return None
        ...
        db.commit()
        func_logger.info(f"Rollback completed successfully for Payment ID: {payment.id}")
        return {"rollback_success": True, "payment_id": payment.id}
    
    except SQLAlchemyError as e:
        db.rollback()
        func_logger.exception(f"DB error during rollback for Payment ID: {payment.id} | Error: {e}")
        raise DBException()

    except Exception as e:
        db.rollback()
        func_logger.exception(f"Unexpected error during rollback for Payment ID: {payment.id}")
        raise DBException()