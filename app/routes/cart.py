...
@cart_router.post('/add-item', response_model=ShowCartItem,status_code=status.HTTP_202_ACCEPTED)
def add_item_to_cart(item: CreateCartItem, user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    func_logger.info(f"POST - /cart/add-item")
    
    cart = CartQueries.get_user_cart(user=user, db=db)
    
    if not cart:
        func_logger.error(f"Cart not available for this user: {user.id}. Creating one...")
        try:
...
def get_all_items(db:Session=Depends(get_db), cart: Cart=Depends(get_cart), user: User=Depends(get_current_user)):
...
def delete_book(
    ...
    try:
        item = CartQueries.get_cart_item_by_id(
            cart_id=cart.id,
            book_id=book_id,
            db=db
        )
        ...
        db.rollback()
        func_logger.exception(f"Failed to delete item from cart: book_id={book_id}, user_id={user.id}")
        raise DBException()
