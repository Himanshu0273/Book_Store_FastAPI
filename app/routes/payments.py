...
from app.utils.enums import PaymentsEnum, TransactionStatusEnum, PaymentMethodEnum, CartActivityEnum
...
from app.exceptions.payment_exceptions import TransactionNotAdded, PaymentNotFound
...
def add_payment(ord_id: int, db: Session = Depends(get_db), current_user: User = Depends(is_customer)):
    try:
        func_logger.info(f"Initiating payment for Order ID: {ord_id} by User ID: {current_user.id}")

        order = OrderQueries.get_order_by_id(user_id=current_user.id, order_id=ord_id, db=db)
        if not order:
            func_logger.warning(f"Order not found: Order ID {ord_id}, User ID: {current_user.id}")
        ...
        if not exists_payment:
            ...
            func_logger.info(f"Created new payment ID: {new_payment.id} for Order ID: {ord_id}")

            transaction = add_transaction(payment=new_payment, db=db)
            if transaction is None:
                func_logger.error(f"Transaction failed to process for Payment ID: {new_payment.id}")
            ...
            if transaction.status == TransactionStatusEnum.SUCCESS:
                ...
                func_logger.info(f"Payment ID: {new_payment.id} and Order ID: {order.id} marked as SUCCESSFUL")
        ...
        if exists_payment.status == PaymentsEnum.SUCCESSFUL:
            func_logger.info(f"Payment ID: {exists_payment.id} already marked SUCCESSFUL — no further action taken.")
            return {
        ...
        if exists_payment.attempts >= 3:
            ...
            db.commit()
            func_logger.warning(f"Payment ID: {exists_payment.id} exceeded 3 attempts and was marked as FAILED")
            rollback_payment(payment=exists_payment, ord_id=ord_id,user_id=current_user.id, db=db)
            raise AttemptsExceeded()


        transaction = add_transaction(payment=exists_payment, db=db)
        if transaction is None:
            func_logger.error(f"Transaction failed to process for existing Payment ID: {exists_payment.id}")
            raise TransactionNotAdded()

        if transaction.status == TransactionStatusEnum.SUCCESS:
            exists_payment.status = PaymentsEnum.SUCCESSFUL
            order.payment_status = PaymentsEnum.SUCCESSFUL
            func_logger.info(f"Payment ID: {exists_payment.id} and Order ID: {order.id} marked as SUCCESSFUL")
        ...
        db.rollback()
        func_logger.exception(f"Database error during payment for Order ID: {ord_id}\nError: {e}")
        raise DBException()

    except Exception as e:
        db.rollback()
        func_logger.exception(f"Unexpected error during payment for Order ID: {ord_id}\nError: {e}")
        raise HTTPException(status_code=500, detail=f"Unexpected error occurred: {e}")


@payment_router.patch("/soft-delete/{payment_id}", status_code=status.HTTP_200_OK)
def soft_delete_payment(payment_id: int, db: Session = Depends(get_db), current_user: User = Depends(is_customer)):
    try:
        func_logger.info(f"Soft-deleting Payment ID: {payment_id} for User ID: {current_user.id}")
        ...
        func_logger.info(f"Marked Payment ID: {payment_id} and Order ID: {order.id} as FAILED; Cart ID: {cart.id} as CANCELLED")
        ...
        for item in cart_items:
            ...
            if not book:
                func_logger.error(f"Book ID {item.book_id} not found while restoring during soft delete.")
        ...
        return {"message": "Payment soft-deleted successfully", "payment_id": payment_id}

    except SQLAlchemyError as e:
        db.rollback()
        func_logger.exception(f"DB error during soft delete for Payment ID: {payment_id} | Error: {e}")
        raise DBException()

    except Exception as e:
        db.rollback()
        func_logger.exception(f"Unexpected error during soft delete for Payment ID: {payment_id}")
